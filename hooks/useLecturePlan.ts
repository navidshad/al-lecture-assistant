...
export function useLecturePlan({ apiKey, selectedLanguage, selectedVoice, selectedModel }: UseLecturePlanOptions): UseLecturePlanResult {
  const [isLoading, setIsLoading] = useState(false);
  const [loadingText, setLoadingText] = useState('');
  const [error, setError] = useState<string | null>(null);

  const createSessionFromPdf = useCallback(async (file: File): Promise<LectureSession> => {
    setIsLoading(true);
    setError(null);
    try {
